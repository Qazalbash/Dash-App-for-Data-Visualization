...
def display_map(selected_year, selected_month, selected_radio):
    ...
    df = pd.read_csv(
        f"D:/4/Data-Structure-II/CS-201-Data-Structure-II-Project/src/data/{selected_year}-{selected_month}.csv",
        dtype={"fips": str})
    ...
    fig = px.choropleth_mapbox(
        ...
        geojson=counties,
        locations='fips',
        color=np.log10(df[selected_radio] + 1),
    ...
    fig.update_layout(
        coloraxis_colorbar=dict(title=f"{selected_year}-{selected_month}",
                                ticktext=[0, df[selected_radio].max()]))
