...
# df = pd.read_csv(
#     'D:/4/Data-Structure-II/CS-201-Data-Structure-II-Project/data/us-county-Abbeville.csv'
# )
...
df_lat_lon = pd.read_csv(
    os.path.join(APP_PATH, os.path.join("us-counties.csv")))
...
df_full_data = pd.read_csv(
    os.path.join(APP_PATH,
                 os.path.join("data",
                              "age_adjusted_death_rate_no_quotes.csv")))
df_full_data["County Code"] = df_full_data["County Code"].apply(
    lambda x: str(x).zfill(5))

df_full_data["County"] = (df_full_data["Unnamed: 0"] + ", " +
    ...
    children=[
        html.Div(
            ...
            children=[
                ...
                html.A(html.Button("Enterprise Demo", className="link-button"),
...
@app.callback(Output("county-choropleth",
...
def display_map(year, figure):
    ...
    layout = dict(mapbox=dict(layers=[],
                              accesstoken=mapbox_access_token,
                              style=mapbox_style,
...
def display_selected_data(selectedData, chart_dropdown, year):
    ...
    if chart_dropdown != "death_rate_all_time":
        ...
        deaths_or_rate_by_fips = deaths_or_rate_by_fips[
            deaths_or_rate_by_fips > 0]
        fig = deaths_or_rate_by_fips.iplot(kind="bar",
                                           y=AGGREGATE_BY,
                                           title=title,
                                           asFigure=True)
    ...
    fig_layout["yaxis"][
        "title"] = "Age-adjusted death rate per county per year"
    ...
    if len(fips) > 500:
        fig["layout"][
            "title"] = "Age-adjusted death rate per county per year <br>(only 1st 500 shown)"
